export const projects = [
    {
        name: 'Recipe Book (MERN)',
        image: '/assets/images/recipe-book-mern.png',
        alt: 'Recipe Book MERN',
        description: 'Web based recipe manager to digitally store, organize, and view recipes',
        technologies: ['HTML', 'CSS', 'JavaScript', 'Tailwind', 'React', 'Node.js', 'Express.js', 'MongoDB',],
        links: {
            demo: 'https://frontend-alpha-one-28.vercel.app/',
            github: 'https://github.com/Rokhai/RecipeBook-Frontend-React'
        }
    },
    {
        name: 'Todo List',
        image: '/assets/images/todo-list.png',
        alt: 'Todo List',
        description: "A simple todo list that allows you to manage tasks, add, edit, and delete them.",
        technologies: ['HTML', 'CSS', 'JavaScript', 'LocalStorage'],
        links: {
            demo: 'https://todo-list-devel.onrender.com/',
            github: 'https://github.com/Rokhai/Todo-List'
        }
    },
    {
        name: 'eXtHealth (Thesis)',
        image: '/assets/images/exthealth.png',
        alt: 'eXtHealth',
        description: 'eXtHealth is a browser extension that fact-checks health claims on X posts and provides timely health tips and reminders',
        technologies: ['HTML', 'CSS', 'JavaScript', 'TypeScript', 'React', 'Node.js', 'Python (FastAPI)', 'Elasticsearch'],
        links: {
            // demo: '',
            github: 'https://github.com/bruce-agaloos/eXtHealth',
            award: '/assets/exthealth-award.pdf'
        }
    },
    // {
    //     name: 'DHVSUM (School Project)',
    //     image: '/assets/images/dhvsum.png',
    //     alt: 'DHVSUM',
    //     description: 'A marketplace website exclusive for Don Honorio Ventura State University Students, and they can buy/sell their pre-owned items, and/or sell their own craft',
    //     technologies: ['HTML', 'CSS', 'JavaScript', 'Bootstrap', 'PHP', 'Laravel', 'MySQL', 'Apache'],
    //     links: {
    //         github: 'https://github.com/Rokhai/dhvsum-v02'
    //     }

    // },


]